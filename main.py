...
def get_user_input():
    ...
    hand_cards = [input("Enter your first card: "), input("Enter your second card: ")]
    table_cards = input("Enter table cards (comma separated, e.g. 2H,3D,4C, leave blank for none): ").split(',')
    num_opponents = int(input("Enter number of opponents (leave blank for none): ") or 0)
...
def index():
    ...
    if request.method == 'POST':
        ...
        table_cards = [request.form.get(f'table{i}') for i in range(1, 6) if request.form.get(f'table{i}')]
